...
struct WizPathRouteInfoPanel: View {
    ...
    var body: some View {
        LiquidGlassCard(accentColor: Color.routeRiskColor(route.overallRisk), innerPadding: 16) {
            VStack(spacing: 14) {
                ...
                // Stats row
                HStack(spacing: 0) {
                    RouteStatItem(icon: "arrow.triangle.swap",
                ...
                if let bestTime = bestDepartureTime,
                   let reason = departureTimeReason {
                    Divider()
...
struct WizPathBestDepartureRow: View {
    let bestTime: Date
    let reason: String
    let onSet: (Date) -> Void
