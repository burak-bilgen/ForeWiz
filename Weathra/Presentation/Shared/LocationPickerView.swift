        ...
        VStack(spacing: 0) {
            HStack {
                if savedLocations.count > 1 {
                    Button(isEditing ? String(localized: "location_picker_done") : String(localized: "location_picker_edit")) {
                        isEditing.toggle()
                    }
                ...
                Button(String(localized: "location_picker_close")) {
                    dismiss()
                }
                .fontWeight(.semibold)
            ...
            if savedLocations.isEmpty {
                ...
                VStack(spacing: AppSpacing.small) {
                    ...
                    Text(String(localized: "location_picker_empty"))
                        .font(AppTypography.headline)
                        .foregroundStyle(AppTheme.ink)
                    Text(String(localized: "location_picker_empty_hint"))
...
private struct AddLocationView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack {
                Button(String(localized: "location_picker_cancel")) { dismiss() }
                Spacer()
                Text(String(localized: "location_picker_add_title"))
                    .font(AppTypography.headline)
                Spacer()
                Button(String(localized: "location_picker_add_button")) { addLocation() }
            ...
            Form {
                Section(String(localized: "location_picker_name_section")) {
                    TextField(String(localized: "location_picker_name_placeholder"), text: $name)
                }

                Section(String(localized: "location_picker_coordinates")) {
                    TextField(String(localized: "location_picker_latitude"), text: $latitudeText)
                        .keyboardType(.decimalPad)
                    TextField(String(localized: "location_picker_longitude"), text: $longitudeText)
                        .keyboardType(.decimalPad)
                }

                Section {
                    Text(String(localized: "location_picker_default_coords_note"))
                        .font(.system(.caption2, design: .rounded))
